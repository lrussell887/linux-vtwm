...
static unsigned short wm8505_ac97_read(struct snd_ac97 *ac97, unsigned short reg)
{
	...
	writel((reg & 0x7f) | CCR_READ_FLAG, priv->regs + WM8505_CCR);
	...
	val = readl(priv->regs + WM8505_CSDR) & 0xffff;
...
static void wm8505_ac97_write(struct snd_ac97 *ac97, unsigned short reg, unsigned short data)
{
	...
	writel((data << 16) | (reg & 0x7f), priv->regs + WM8505_CCR);
...
/**
 * wm8505_ac97_dai_probe - initialise DAI DMA data
 */
...
/**
 * wm8505_ac97_machine_init - board-level codec initialisation
 */
static int wm8505_ac97_machine_init(struct snd_soc_pcm_runtime *rtd)
{
	/*
	 * Enable VT1613 Headphone Amplifier (fixes impedance mismatch/clipping).
	 * De-emphasis (DE) is omitted to prevent high-frequency muffling.
	 */
	wm8505_ac97_write(NULL, VT1613_FCS1,
	                  wm8505_ac97_read(NULL, VT1613_FCS1) | VT1613_FCS1_HPEN);
	wm8505_ac97_write(NULL, VT1613_IODA, 0x401);
	wm8505_ac97_write(NULL, VT1613_EINT, 0x0);
	wm8505_ac97_write(NULL, VT1613_PLL,  0x84);
...
static int wm8505_ac97_probe(struct platform_device *pdev)
{
	...
	/* Set FIFO thresholds to 8 (matches DMA maxburst) */
	writel(0x800, priv->regs + WM8505_PTCR);
	writel(0x800, priv->regs + WM8505_PRCR);
	...
	priv->capture_dma_data.maxburst    = 8;
	...
	if (IS_ERR(priv->ac97_pdev)) {
		...
		return dev_err_probe(&pdev->dev, PTR_ERR(priv->ac97_pdev), "Failed to register ac97-codec\n");
	}
	...
	dev_info(&pdev->dev, "WM8505 AC97 Audio Subsystem initialized successfully\n");
	return 0;

err_pdev:
	platform_device_unregister(priv->ac97_pdev);
	snd_soc_set_ac97_ops(NULL);
	return ret;
}

/**
 * wm8505_ac97_remove - platform device remove
 */
static void wm8505_ac97_remove(struct platform_device *pdev)
{
	struct wm8505_ac97 *priv = platform_get_drvdata(pdev);
