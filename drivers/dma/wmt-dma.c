 ...
 * The DMA controller acts as a master to fetch its runtime context
...
/**
 * wmt_dma_slave_config - DMA slave configuration callback
 */
static int wmt_dma_slave_config(struct dma_chan *chan, struct dma_slave_config *cfg)
{
...
static void wmt_dma_start_desc(struct wmt_dma_chan *c)
{
	...
	if (desc->dir == DMA_DEV_TO_MEM)
		dmadev->mem_regs[c->id].if1_cpr = c->slave_cfg.src_addr;
	else
		dmadev->mem_regs[c->id].if1_cpr = c->slave_cfg.dst_addr;

	writel(BIT(c->id), dmadev->base + WMT_DMA_ISR);

	/* Ensure memory writes to Context RAM are flushed before execution */
	wmb();
...
/**
 * wmt_dma_get_ccr - build a channel control register value from slave config
 */
static u32 wmt_dma_get_ccr(struct wmt_dma_chan *c, enum dma_transfer_direction dir)
{
	...
	enum dma_slave_buswidth buswidth = (dir == DMA_DEV_TO_MEM) ?
		c->slave_cfg.src_addr_width : c->slave_cfg.dst_addr_width;
	u32 maxburst = (dir == DMA_DEV_TO_MEM) ?
		c->slave_cfg.src_maxburst : c->slave_cfg.dst_maxburst;
 ...
 */
static struct dma_async_tx_descriptor *wmt_dma_prep_dma_cyclic(
	...
	size_t period_len, enum dma_transfer_direction dir, unsigned long flags)
{
	...
	/* Prevent buffer overflow against the 128-descriptor DMA pool */
	if (periods > 128) {
 ...
 */
static struct dma_async_tx_descriptor *wmt_dma_prep_slave_sg(
	...
	if (c->active_desc) {
		if (evt == WMT_DMA_EVT_SUCCESS || evt == 0) {
			if (c->active_desc->cyclic) {
				struct virt_dma_desc *vd = &c->active_desc->vdesc;
				spin_unlock_irqrestore(&c->vchan.lock, flags);
...
static int wmt_dma_probe(struct platform_device *pdev)
{
	...
	dmadev->mem_regs = dmam_alloc_coherent(dev, sizeof(struct wmt_dma_mem_reg) * WMT_DMA_CHANNELS,
	...
	dmadev->desc_pool = dmam_pool_create("wmt_dma_desc", dev,
	                                     sizeof(struct wmt_dma_hw_desc) * 128,
	                                     16, 0);
	if (!dmadev->desc_pool)
		return -ENOMEM;
	...
	dmadev->ddev.device_config = wmt_dma_slave_config;
