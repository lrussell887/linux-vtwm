...
/*
 * =============================================================================
 * Display Pipeline (GOVR) Configuration
 * =============================================================================
 */
...
{
	/* Disable timing generator during configuration */
	...
	for (u32 div = 1; div <= 31 && best_diff; div++) {
		long pll = clk_round_rate(parent, t_rate * div);
		if (pll > 0) {
	...
	/* Apply the divider */
	writel((max_t(u32, DIV_ROUND_CLOSEST(clk_get_rate(wmt->clk), t_rate), 1) - 1) & 0x7f,
	       wmt->govr_regs + WMT_GOVR_READ_CYC);

	/* Display geometry */
	int h_sync = mode->hsync_end - mode->hsync_start;
	int h_bp = mode->htotal - mode->hsync_end;
	int h_fp = mode->hsync_start - mode->hdisplay;

	int v_sync = mode->vsync_end - mode->vsync_start;
	int v_bp = mode->vtotal - mode->vsync_end;
	int v_fp = mode->vsync_start - mode->vdisplay;

	int h_start = h_sync + h_bp;
	int h_end = h_start + mode->hdisplay;
	int h_all = h_end + h_fp;

	int v_start = v_sync + v_bp + 1;
	int v_end = v_start + mode->vdisplay;
	int v_all = v_end + v_fp - 1;
...
/**
 * wmt_drm_irq - IRQ handler to detect hardware vblank signals
 */
...
static void wmt_pipe_disable(struct drm_simple_display_pipe *pipe)
{
	...
	if (wmt->pending_event) {
		drm_crtc_send_vblank_event(&pipe->crtc, wmt->pending_event);
		drm_crtc_vblank_put(&pipe->crtc);
		wmt->pending_event = NULL;
	}
...
/**
 * wmt_pipe_enable_vblank - Vblank enable callback
 */
...
/**
 * wmt_pipe_disable_vblank - Vblank disable callback
 */
...
/**
 * wmt_pipe_update - Plane update callback for page-flips
 */
			    ...
			    struct drm_plane_state *old_state)
{
	...
	if (pipe->crtc.state->event) {
		...
		wmt->pending_event = pipe->crtc.state->event;
		pipe->crtc.state->event = NULL;
		wmt->defer_vblank = status & WMT_VPP_VBIS_BIT;

		if (drm_crtc_vblank_get(&pipe->crtc) != 0) {
			drm_crtc_send_vblank_event(&pipe->crtc, wmt->pending_event);
			wmt->pending_event = NULL;
			wmt->defer_vblank = false;
		}
